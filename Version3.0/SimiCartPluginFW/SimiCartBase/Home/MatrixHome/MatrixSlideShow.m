...
@implementation MatrixSlideShow
...
- (void) setDefaultValues
{
    ...
    self.arrayImageView = [NSMutableArray new];
...
- (void) next
{
    if(! _isAnimating){
        ...
        if (self.useImages) {
            nextIndex = (_currentIndex+1)%[self.images count];
            _topImageView.image = self.images[_currentIndex];
            _bottomImageView.image = self.images[nextIndex];
        }else
        {
            UIImageView *imageView = [self.arrayImageView objectAtIndex:_currentIndex];
            if (imageView.image) {
                [_topImageView setImage:imageView.image];
            }
            imageView = [self.arrayImageView objectAtIndex:nextIndex];
            if (imageView.image) {
                _bottomImageView.image = imageView.image;
            }
        }
...
- (void) previous
{
    if(! _isAnimating){
        ...
        }else
        {
            ...
            imageView = [self.arrayImageView objectAtIndex:prevIndex];
            if (imageView.image) {
                _bottomImageView.image = imageView.image;
            }
        }
